local MAJOR, MINOR = "LibFail-1.0", tonumber("243") or 999
...
function lib:SPELL_DAMAGE(timestamp, type, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellId, spellName, spellSchool, damage, overkill)
   ...
   -- Gunship -- Explosion (it's still avoidable so optional fail for normal mode)
    ...
    -- Festergut Heroic -- Malleable goo (Damage)
    if (spellId == 72549 or spellId == 72550) and is_playerevent and overkill > 0 then
        self:FailEvent("Fail_Festergut_MalleableGoo", destName, self.FAIL_TYPE_NOTMOVING)

        return
    end

    -- Professor - Malleable Goo (Damage)
    if (spellId == 70853 or spellId == 72458 or spellId == 72873 or spellId == 72874) and is_playerevent and overkill > 0 then
